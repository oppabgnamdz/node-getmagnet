...
router.get('/ffjav', async (req, res) => {
	...
	try {
		...
		// Mặc định là 5 trang để giảm tải tài nguyên, có thể thay đổi qua tham số query
		const pagesToCrawl = req.query.pages
			? parseInt(req.query.pages as string)
			: 5;
		console.log(`Sẽ crawl ${pagesToCrawl} trang từ ffjav.com/jav-torrent`);

		// Thử sử dụng puppeteer nếu có thể
		try {
			// Khởi tạo puppeteer với cấu hình tối ưu nhằm giảm tài nguyên
			browser = await puppeteer.launch({
				headless: true, // Sửa 'new' thành true để tương thích với các phiên bản cũ hơn
				executablePath: process.env.PUPPETEER_EXECUTABLE_PATH || undefined,
				args: [
					'--no-sandbox',
					'--disable-setuid-sandbox',
					...
					'--disable-features=IsolateOrigins,site-per-process',
					'--disable-web-security',
					'--no-zygote', // Giữ lại tùy chọn này để tránh các vấn đề về quyền truy cập
					'--js-flags=--max-old-space-size=256', // Giảm giới hạn bộ nhớ JavaScript xuống
					'--disable-extensions',
					'--disable-component-extensions-with-background-pages',
					'--disable-default-apps',
					'--mute-audio',
				],
				defaultViewport: { width: 800, height: 600 }, // Giảm kích thước cửa sổ
			...
			for (let page = 1; page <= pagesToCrawl; page++) {
				...
				try {
					// Tối ưu hiệu suất cho trang
					await pageBrowser.setCacheEnabled(true);
					await pageBrowser.setJavaScriptEnabled(true); // Cần JavaScript để tải trang
					...
					// Cấu hình trình duyệt giống người dùng thật nhưng đơn giản hóa
					await pageBrowser.setUserAgent(
						'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
					);

					// Truy cập trang với cài đặt đợi tối ưu
					console.log(`Đang truy cập trang: ${pageUrl}`);
					await pageBrowser.goto(pageUrl, {
						waitUntil: 'domcontentloaded', // Chỉ đợi DOM load, không đợi tất cả requests
						timeout: 30000, // Giảm timeout xuống 30 giây
					});
				...
				// Giảm delay xuống còn 500ms để tăng hiệu suất
				if (page < pagesToCrawl) {
					await new Promise((resolve) => setTimeout(resolve, 500));
				}
			}

			// Đóng browser ngay sau khi đã lấy xong tất cả links
			if (browser) {
				await browser.close();
				browser = null;
			}

			// Nếu tìm thấy links, xử lý và trả về kết quả
			if (allDownloadLinks.length > 0) {
				return await processAndSaveLinks(allDownloadLinks, collection, res);
			}
		} catch (puppeteerError: any) {
			// Nếu puppeteer gặp lỗi, ghi log và chuyển sang dùng axios
			console.error(
				'Puppeteer không khả dụng, chuyển sang dùng axios:',
		...
		// Phương án dự phòng: Sử dụng axios nếu puppeteer không khả dụng
		console.log('Sử dụng axios để crawl dữ liệu từ ffjav.com');

		// Mảng chứa các download links
		let allDownloadLinks: string[] = [];

		// Xử lý từng trang bằng axios
		for (let page = 1; page <= pagesToCrawl; page++) {
			console.log(
				`\n==== Đang xử lý trang ${page}/${pagesToCrawl} bằng axios ====`
			);
			...
			try {
				...
				// Tìm tất cả các href có chứa /download/2025/
				const links: string[] = [];
				$('a[href*="/download/2025/"]').each((_, element) => {
				...
				console.error(
					`Lỗi khi xử lý trang ${page} bằng axios:`,
					axiosError.message
				);
			}
		...
		// Xử lý và lưu các links tìm được
		return await processAndSaveLinks(allDownloadLinks, collection, res);
	} catch (error: any) {
...
// Hàm xử lý và lưu links vào database
async function processAndSaveLinks(
	allDownloadLinks: string[],
	collection: any,
	res: any
) {
